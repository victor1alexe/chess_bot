...
// 	Get the move from the command
    ...
    if (string_move.size() == 5){
        special_move s_m;
        switch (command[13]) {
            case 'r': s_m = PROMOTION_ROOK; break;
            case 'n': s_m = PROMOTION_KNIGHT; break;
            case 'b': s_m = PROMOTION_BISHOP; break;
            default: s_m = PROMOTION_QUEEN; break;
        }
        move m = move({toupper(string_move[0]), toupper(string_move[1])}, {toupper(string_move[2]), toupper(string_move[3])}, s_m);
        b.make_move(m);
        made_move = true;
    }
    ...
    if (m.get_from().second == '@' && !made_move) {
